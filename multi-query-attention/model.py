...
class SelfAttentionHead(nn.Module):
    ...
    def forward(self, x, key_layer, value_layer):
        # x.shape: (Batch, Context Length, Embedding Dimension)
        B, C, N = x.shape
        q = self.query(x)  # (B, C, head_size)
        k = key_layer(x)  # (B, C, head_size)
        v = value_layer(x)  # (B, C, head_size)
...
class MultiQueryAttention(nn.Module):
    def __init__(self, config):
        ...
        self.keys = nn.Linear(config.n_embed, config.head_size, bias=False)
        self.values = nn.Linear(config.n_embed, config.head_size, bias=False)
    ...
    def forward(self, x):
        multihead_output = torch.cat(
            [head(x, self.keys, self.values) for head in self.heads], dim=-1
        )
...
class GPTBlock(nn.Module):
    def __init__(self, config):
        ...
        self.multiheaded_attn = MultiQueryAttention(config)
        self.ffn = FFN(config)
